...
class CellDreamerEstimator:
    def __init__(self, args):
        ...
        val_metric_collector = MetricsCollector(
            self.datamodule.val_dataloader,
            self.args.task,
            self.feature_embeddings)

        self.metric_collector = {"train": train_metric_collector,
                                 "valid": val_metric_collector}
    ...
    def init_datamodule(self):
        ...
        """
        assert self.args.task in ["cell_generation", "perturbation_modelling",
                                  "toy_generation"], f"The task {self.args.task} is not implemented"
        ...
        if self.args.task == "cell_generation":
            if self.args.atlas == "pbmc":
                ...
                train_data, test_data, val_data = random_split(self.dataset, lengths=self.args.split_rates)
    ...
    def get_fixed_model_params(self):
        ...
        if self.args.task == "perturbation_modelling":
            if self.args.use_latent_repr:
                ...
                self.args.denoising_module_kwargs["in_dim"] = self.dataset.genes.shape[
                    1]  # perform diffusion in gene dimension
        elif self.args.task == "cell_generation":
            if self.args.atlas == "cellxgene":
                self.args.denoising_module_kwargs["in_dim"] = len(
                    pd.read_parquet(join(self.args.data_path, 'var.parquet')))
    ...
    def train(self):
        ...
        self._check_is_initialized()  # check that the model, datamodule and trainer are initialized
